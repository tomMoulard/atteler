...
func providerRuntimeCatalog() map[string]ProviderRuntimeInfo {
	return map[string]ProviderRuntimeInfo{
		providerAnthropic: { // #nosec G101 -- documentation names credential sources, not secret values.
			...
			TokenRefresh:     "ForgeCode OAuth credentials refresh during credential resolution only when `credential_policy.allow_refresh` and `allow_write_back` allow it; the Anthropic adapter itself does not refresh on 401.",
		...
		providerClaudeCode: { // #nosec G101 -- documentation names credential sources, not secret values.
			...
			TokenRefresh:     fmt.Sprintf("On 401, exchanges the stored refresh token at `%s` and persists refreshed tokens back to the same Claude Code credential store only when `credential_policy.allow_refresh` and `allow_write_back` allow it.", claudeCodeRefreshURL),
		...
		providerCodex: { // #nosec G101 -- documentation names credential sources, not secret values.
			...
			TokenRefresh:     fmt.Sprintf("On 401, exchanges the stored refresh token at `%s` and atomically updates `auth.json` only when `credential_policy.allow_refresh` and `allow_write_back` allow it.", codexChatGPTRefreshURL),
