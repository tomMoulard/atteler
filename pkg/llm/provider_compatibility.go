...
func ProviderCompatibilityFor(providerName string) (ProviderCompatibilityRow, bool) {
	...
	case providerAnthropic:
		...
		row.ShellAccess = compatibilityCell("none", "no subprocess, CLI, or workspace shell/tool sandbox")
	...
	case providerClaudeCode:
		...
		row.ShellAccess = compatibilityCell("none", "does not run the Claude Code CLI or expose its file/search/edit tools")
		row.ContextWindow = compatibilityCell("static", "static Claude Code adapter catalog with reviewed context-window assumptions")
	...
	case providerCodex:
		...
		row.ShellAccess = compatibilityCell("none", "does not run `codex exec` or expose Codex CLI workspace tools")
		row.ContextWindow = compatibilityCell("static+unknown-overrides", "static Codex catalog for known IDs; user-configured model overrides intentionally return unknown context")
	...
	case providerOllama:
		...
		row.ShellAccess = compatibilityCell("daemon-autostart", "no shell/tool access; may start `ollama serve` only when auto-start is explicitly enabled")
