...
var cliHelpDomains = []cliHelpDomain{
	...
	},
	{
		...
		Commands: []cliCommandAlias{
			...
			{Name: "delete", Args: "<id>", Summary: "delete one document from the local memory store", Legacy: []string{"--memory-delete"}},
			{Name: "compact", Summary: "remove expired documents from the local memory store", Legacy: []string{"--memory-compact"}},
			{Name: "migrate", Summary: "explicitly migrate and redact the local memory store", Legacy: []string{"--memory-migrate"}},
