...
func TestREADME_CLIDocumentationDefersToGeneratedHelp(t *testing.T) {
	...
	assert.LessOrEqual(t, len(legacyFlagMentions), 105)
}
...
func TestFlagDomain_CoversAcceptanceDomains(t *testing.T) {
	...
	tests := map[string]string{
		...
		"retry-headless":                     "chat/session",
		"retry-headless-id":                  "chat/session",
		"stream-headless":                    "chat/session",
		"cleanup-headless":                   "chat/session",
		"headless-status":                    "chat/session",
		"headless-max-age":                   "chat/session",
