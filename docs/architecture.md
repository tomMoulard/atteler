...
providers are `anthropic`, `claude-code`, `codex`, `openai`, and `ollama`.
Codex and Claude Code reuse locally stored OAuth credentials, but completions
are direct HTTPS calls from atteler rather than shelling out to those CLIs.
...
accumulate. The built-in Anthropic, Claude Code, Codex, Ollama, and OpenAI
providers expose native streaming adapters; custom providers use
