...
For vision-capable models, `@path` can also attach local JPEG, PNG, GIF, or
WebP images. Image bytes are sent as typed multimodal content while the visible
prompt only records an `<image ... attached="true">` placeholder:

```sh
atteler chat once "Explain the error dialog in @screenshots/failure.png"
```
