...
`symphony --validate` is strict about workflow front matter. Unknown keys fail
validation before Symphony starts polling, opening workspaces, publishing PRs,
or removing labels. Error messages use dotted paths such as
`publish.monitor_checks` and include a closest-key hint when the typo is clear.
...
Supported nested keys are:

- `tracker`: `kind`, `endpoint`, `api_key`, `project_slug`, `repository`,
  `owner`, `repo`, `active_states`, `terminal_states`, `labels`
- `workspace`: `root`
- `publish`: `enabled`, `remote`, `remote_url`, `base_branch`,
  `branch_prefix`, `draft`, `draft_on_failed_validation`, `monitor_checks`,
  `verification_gates`, `verification_allow_commands`,
  `verification_deny_commands`, `verification_output_max_bytes`,
  `remove_labels`, `required_checks`, `required_check_patterns`,
  `discover_required_checks`, `no_checks_policy`, `rework_optional_checks`,
  `check_interval_ms`, `max_check_rework_attempts`, `git_user_name`,
  `git_user_email`
- `publish.verification_gates` map entries: `name`, `command`, `required`,
  `timeout_ms`
- `debug`: `enabled`, `address`, `event_limit`
...
If local tooling needs metadata that Symphony itself should ignore, place it
under an `extensions` map or prefix the key with `x_` at the relevant mapping
level. Arbitrary Codex app-server config belongs under `codex.config`; keys
there are passed through as the app-server thread `config` payload instead of
being interpreted as Symphony settings.
...
- `codex.config` is an explicit pass-through map for Codex app-server thread
  config. Put arbitrary app-server options there; misspelled Symphony config
  keys under `codex` are rejected by workflow validation.
