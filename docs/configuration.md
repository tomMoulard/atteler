...
agents:
  reviewer:                                  # any agent name
    ...
    tool_policy: deny                        # default: deny; allow-all is explicit legacy compatibility
    tools:                                    # default: omitted/{} (no tools allowed)
      read: true                             # capability grant: read, glob, grep
      search: true                           # capability grant: glob, grep
      shell.readonly: false                  # capability grant: read-only bash commands only
...
Capability or tool allow/deny map. Prefer capability-scoped grants (`read`,
`search`, `shell.readonly`, `shell.write`, `network`, `filesystem.write`) over
raw tool names. `shell.readonly` advertises bash but denies mutating shell commands at runtime,
and shell network commands also require `network: true`. Unlisted tools are denied unless
`tool_policy: allow-all` is set. Explicit `false` entries take precedence over
overlapping raw-tool or capability grants, so a broad deny such as
`filesystem.write: false` blocks `write: true` and `edit: true`.

- **Default:** omitted or `{}` denies every tool. Use explicit capability grants.
- **Compatibility:** set `tool_policy: allow-all` only when a legacy agent should receive every advertised tool, including future tools.

```yaml
agents:
  reviewer:
    tools:
      read: true             # read, glob, grep
      search: true           # glob, grep
      network: false         # network-capable shell commands
      filesystem.write: false
      shell.readonly: false
```
