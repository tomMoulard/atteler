...
atteler agents spawn 'reviewer|check the diff' \
  ...
  --spawn-task-timeout-seconds 120 \
  --spawn-retries 1 \
  --spawn-retry-backoff-seconds 5
...
without rerunning matching successful children, and tune backpressure with
`--spawn-max-concurrency`, `--spawn-task-timeout-seconds`, `--spawn-retries`,
`--spawn-retry-backoff-seconds`, `--spawn-cancel-on-failure`, `--spawn-token-budget`,
`--spawn-cost-budget-micros`, and `--spawn-output-budget-bytes`. Child processes
